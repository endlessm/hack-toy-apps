      ...
      switch (data->header.type)
        {
          ...
          case CAIRO_PATH_LINE_TO:
            ...
            break;
          case CAIRO_PATH_CURVE_TO:
            g_message ("Curves are not suported in json format");
            break;
          case CAIRO_PATH_CLOSE_PATH:
            break;
        }
...
static gchar *
path_data_append_json_shape (GString *string,
                             gchar *name,
                             cairo_path_data_t *path_data,
                             gint size,
                             gdouble width,
      ...
      switch (data->header.type)
        {
          case CAIRO_PATH_MOVE_TO:
          case CAIRO_PATH_LINE_TO:
            g_string_append_printf (string, "%s    { \"x\": %lf, \"y\": %lf }",
                                    (i == 0) ? "" : ",\n",
                                    data[1].point.x,
                                    data[1].point.y);
            break;
...
/* gcc svg2path.c -o svg2path `pkg-config --libs --cflags glib-2.0` -lm */
...
main (int argc, char **argv)
{
  ...
  gchar *basename;
  GString *string;
  gboolean dump_header, dump_shape, dump_poly;
  gint i;

  context = g_option_context_new ("FILES - Convert path data in a SVG to a cairo_path_t or json list of vertices");
  ...
  dump_header = g_str_has_suffix (basename, ".h");
  dump_shape  = g_str_has_suffix (basename, ".shape.json");
  dump_poly  = g_str_has_suffix (basename, ".polygon.json");
